...
### Q4: Why are the individual frontend and backend Kubernetes Services configured as `ClusterIP` only?
* **Single Gatekeeper (The Ingress / API Gateway Pattern)**: We use a dedicated **Nginx Ingress Controller** as the single public-facing entry point to the cluster. The Ingress sits at the edge of the network, handles domain routing (`semantic-analysis.test`), strips paths (e.g. `/api` rewrite), and routes external traffic to the internal pods. Because Ingress handles the external entry point, the individual microservices do not need to be exposed directly to the outside world.
* **Security & Attack Surface Reduction**: If we used `NodePort` or `LoadBalancer` for the backend, port `8000` (or a random port like `30000+`) would be exposed directly to the internet. This would allow anyone to bypass our Ingress routing policies, access controls, and logs to query our ML model directly. By using `ClusterIP`, the services are assigned internal private IPs that can *only* be accessed by components inside the cluster's software-defined network (like Ingress or other internal pods).
* **Lightweight Internal Communication**: `ClusterIP` is the default and most performant service type in Kubernetes. It creates a stable internal virtual IP and register DNS entries (like `http://semantic-backend` and `http://semantic-frontend`), allowing seamless, low-overhead communication between containers inside the cluster network.
